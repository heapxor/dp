...
% 
% V dnešnej dobe existuje veľký počet aplikácií, ktoré nie su až tak kritické pre zápis a na ich dátach sa vykonáva množstvo operácií, ktoré dáta čítajú a následne spracúvajú. Patria sem data warehouse, customer relationship management (CRM) systémy, systémy pre vyťažovanie dát alebo analytické aplikácie aplikované na obrovské objemy dát. Pre túto potrebu vznikol stĺpcový model [13][14] reprezentácie dát, ktorý je optimalizovaný pre čítanie dát a je vhodný pre OLAP. Dáta sú uložené v stĺpcoch a súvisle uložené na disku. Načítanie podobných dát (stĺpec) do pamäti a následná práca s nimi je efektivnejšia ako u riadkových databázy, kde je potrebné načítať celý záznam obsahujúci hodnoty stĺpcov, ktoré pre nás sú v daný moment irelevantné.
% 
