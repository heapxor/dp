...
Pojem NoSQL bol prvykrát použitý v roku 1998 ako názov relačnej databázy, ktorej implementácia bola prevažne v interpretovaných programovacích jazykoch a neobsahovala SQL interprét. NoSQL (interpretovaný taktiež ako Not Only SQL), sa v druhej polovici roku 2009 [4] začal používať v spojení s databázovými systémami, ktoré nepouživajú SQL dotazovací jazyk a tradičný relačný model, sú schopné horizontálneho rozširovania (tj. distribuovaný systém), pracujú na bežných spotrebných počítačoch a vyznačujú sa vysokou dostupnosťou.

Túto generáciu databáz je možné charakterizovať pojmamy ako non-relational, distribuovaný, open-source a horizontálne škálovateľný systém. 

Novo vznikajúce systémy ako napríklad sociálne siete spracúvajú čoraz väčší objem dát a zároveň musia byť schopné v daný moment obslúžiť čoraz väčší počet užívateľov. Pôvodným cieľom hnutia nosql, bolo vytvoriť koncept, pre tvorbu moderných web-scale databáz, ktoré by boli schopné riešiť tieto nové požiadavky.  Idea týchto systémov je založená filozofii, ktorá hovorí, že nemá zmysel sa za každú cenu snažiť prispôsobiť vstupné dáta modelu relačnej databáze. Cieľom je vybrať systém, ktorý bude čo najvhodnejšie opovedať požiadavkom na uloženie a spracovanie našich dát. NoSQL obecne nepopisuje, žiaden konkrétny databázový systém, namiesto toho je to obecný pojem pre non-relational databázové systémy, ktoré majú rôzne charakteristiky, architektúru a umožnujú prácu s rôznými dátovými modelmi. Medzi ich hlavné znaky patrí vysoká dostupnosť, škálovateľnosť, čiastočná konzistencia, možnosť spracúvať obrovské objemy dát (PB), bezschémová reprezentácia dát a jednoduché API. Tieto systémy nepodporujú operáciu join z architektonických dôvodov (preco?). Medzi dátami, ktoré do nich ukladáme, je možné riešiť vzájomné závislosti až na aplikačnej vrstve. Cieľom tohoto konceptu je riešiť spomínané novo vznikajúce problémy a zároveň koexistovať s relačnými databázovými  systémami.

neposkytuju mozost modelovat zavislosti medzi ulozenými datami, toto je nutne riesit na aplikacnej vrstve. kazda z nich je urcena na riesenie specifickeho problemu podla architektury db stroja.
...
Typy NoSQL databázových systémov

Obecne pre tieto db platí, že sú distribuované a natívne podporují replikáciu. Kedže sa jedná o relatívne mladé systémy, jedným z ich nedostatkov je malá podpora frameworkov, neustále sa meniace API a taktiež u mnohých chýbajúce rôzne grafické utility pre ich správu a monitoring.
Key-value

Tento model pre ukladanie dát využíva jednoduchý princíp. Dáta v ľubovoľnej forme sú uložené v databáze pod kľúčom, ktorý môžeme reprezentovať ľubovoľným reťazcom, celočíselným typom a pod. V relačnom modeli dáta reprezentujeme pomocou tabuliek, pre ktoré definujeme ich štruktúru (názvy stĺpcov) a ktoré sú následne normalizované aby sme predišli duplikácii dát. Pre zabezpečenie integrity jednotlivých entít a referenčnej integrity využívame primárne a cudzie kľúče. Tabuľky s popisom názvov ich stĺpcov a vzťahy medzi nimi nazývame databázovou schémou. Úložiško typu kľúč-hodnota nevyužíva normalizáciu dát, dáta sú často duplikované, vzťahy medzi nimi sa riešia až na aplikačnej úrovni. Pre vkladané data a k nim asociované kľúče sa nedefinujú žiadne obmedzenia.

Tento model sa často kombinuje so stĺpcovým modelom, kde sa na dáta pozeráme ako na viacdimenzionálnu hash-mapu.

Daný model využívajú databáze ako Dynamo, Tokyo Cabinet, Redis a iné.
Stĺpcové databáze

Množstvo databáz využíva pre reprezentáciu dát tabuľky, ktoré sú tvorené stĺpcami a presne popísané schémou (tj. názvy stĺpcov, tabuliek). Každý riadok tabuľky reprezentuje dáta, ktoré sa nazývajú záznamy a tieto sú následne sekvenčne ukladané na disk. Tento model, nazývaný riadkový, je vhodný pre systémy, ktoré mnozštvo operácií využivajúcich zápis - relačné databáze sú teda optimalizované pre zápis.  Pre efektívny prístup k dátam sa využívajú indexy.

V dnešnej dobe existuje veľḱy počet aplikácií, ktoré nie su až tak kritické pre zápis a na ich dátach sa vykonáva množstvo operácií, ktoré dáta čítajú a následne spracúvajú. Patria sem data warehouse, customer relationship management (CRM) systémy, systémy pre vyťažovanie dát alebo analytické aplikácie aplikované na obrovské objemy dát. Pre túto potrebu vznikol stĺpcový model [13][14] reprezentácie dát, ktorý je optimalizovaný pre čítanie dát a je vhodný pre OLAP. Dáta sú uložené v stĺpcoch a súvisle uložené na disku. Načítanie podobných dát (stĺpec) do pamäti a následná práca s nimi je efektivnejšia ako u riadkových databázy, kde je potrebné načítať celý záznam obsahujúci hodnoty stĺpcov, ktoré pre nás sú v daný moment irelevantné.

Riadkový model obsahuje v jednom zázname dáta z rôznych domén, čo spôsobuje vyššiu entropiu v porovnaní so stĺpcovým modelom, kde sa predpokláda, že dáta v danom stĺpci sú si podobné. Táto vlasnosť umožnuje efektívnu komprimáciu dát, ktorá znižuje počet diskových operácií za cenu CPU.

Tento model v spojení s následujúcim modelom kľúč-hodnota využívajú hlavne databázové systémy ako Google BigTable, Hadoop Hbase alebo Cassandra.

=zlozity update - load do ram, mergovanie...

=priklad pre ulozenie riad-stlpcovy model
Dokumentové

Dokumentové databáze využívajú princípy predchádzajúceho úložiska typu kľúč-hodnota. Dáta sú reprezentované pomocou dokumentov. Dokument obsahuje neobmedzený počet polí, ku ktorým priradzujeme rôzne hodnoty. Štruktúra dokumentu môže byť popísaná pomocou XML, JSON, YAML atď.a vďaka nej sme schopný v týchto dokumentoch efektívne vyhľadávať.

Medzi databáze reprezentujúce tento typ úložiska patrí napríklad CouchDB a MongoDB.
Grafové

Tento typ databáz využíva pre ukladanie dát matematickú štruktúru - graf. Dáta sú reprezentované pomocou uzlov, hran a ich atribútov. Uzol je základný samostatný a nezávislý objekt. Pomocou hran medzi uzlami modelujeme závislosti, ktoré popisujeme pomocou atribútov. Medzi hlavné výhody patrí možnosť prechádzania týchto štruktúr s využitím známych grafových algoritmov. Tento model sa napríklad využíva v aplikáciach sociálnych sieti alebo pre sémantický web. Patria sem databáze ako Neo4j alebo FlockDB.
...
Archivácia elektronickej pošty
    ...
    * e-discovery  ??? strucne popisat jednotlive pojmy
...
Požiadavky na systém
    ...
    V následujúcej časti popíšeme vlastnosti systému, ktorý bude umožnovať archiváciu veľkého objemu emailových správ.
Obecne vlastnosti

Primárnym požiadavkom na systém je jeho rozšíriteľnosť, dostupnosť a nízkonakladová administrácia. Predpokladané množstvo uložených dát v tomto systéme bude dosahovať desiatky až stovky TB (terabyte). Takúto kapacitu dát nie je možne uchovať na bežne prístupnom spotrebnom hardware (ang. commodity hardware). Systém musí byť neustále dostupný a dáta zalohované v prípade vzniku havárie niektorej z jeho časti. Nad uloženými dátami, je potrebné vykonávať výpočtovo náročné operácie napr. generovanie štatistík. Tieto požiadavky prirodzene implikujú využitie distribuovaného databázového systému. Medzi hlavných kandidátov, vďaka ktorým sme tieto požiadavky schopný vyriešiť patria noSQL databázové systémy, ktorých zrovnanie popíšeme v následujúcej kapitole.
Rozšíriteľnosť

Predpokladáme použitie bežne dostupného spotrebného hardware, namiesto superpočítačov. Systém musí byť schopný horizontálneho škálovania, ktoré využijeme pre zvýšenie celkovej kapacity dátového úložiska systému a na prípadné efektívnejšie spracovanie dát pomocou techniky MapReduce. U databazového systému je nutná podpora replikácie, ktorá zvýši výkonnosť operácií pre čítanie a zápis do systému a vďaka nej nebude nutné riešiť zálohovanie systému pomocou externého riešenia.
Dostupnosť
